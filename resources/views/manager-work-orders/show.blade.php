                                    ...
                                    <div class="card">
                                        <div class="card-header" id="heading{{ $revision->id }}">
                                            <h5 class="mb-0">
                                                ...
                                                @if ($workOrder->status !== 'Facturado')
                                                    <form action="{{ route('work-orders.remove-revision', [$workOrder->id, $revision->id]) }}" method="POST" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-danger btn-sm float-right">Eliminar Revisión</button>
                                                    </form>
                                                @endif
                                            </h5>
                                        </div>
                    ...
                    <div class="row no-print">
                        <div class="col-12">
                            ...
                            @if ($workOrder->status !== 'Facturado' && $workOrder->status !== 'No Realizado')
                                ...
                                <button onclick="marcarNoRealizado()" class="btn btn-danger float-right" style="margin-right: 10px;">
                                    <i class="fas fa-times-circle"></i> No Realizado
                                </button>
                                <form action="{{ route('manager-work-orders.close', $workOrder->id) }}" method="POST" class="float-right" style="margin-right: 10px;">
                                    @csrf
                                    <button type="submit" class="btn btn-dark">
                                        <i class="fas fa-lock"></i> Cerrar OT
                                    </button>
                                </form>
                            @endif
                        </div>
